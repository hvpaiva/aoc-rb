...
class CommandsTest < Minitest::Test
  ...
  def test_runner_check_runs_syntax_standard_and_runner_tests
    commands = []
    command_runner = lambda do |*command|
      commands << command
      true
    ...
    assert(commands.any? { |command| command.first(3) == %w[bundle exec standardrb] })
    assert(commands.any? { |command| command.include?("-rminitest/pride") })
    assert(commands.any? { |command| command.include?("-Irunner/test") })
  end

  def test_check_lints_solutions_with_standard_only
    Dir.mktmpdir do |dir|
      paths = AOC::Paths.new(root: dir, config_dir: File.join(dir, "config"))
      write_file(paths.day_path(2024, 2), "# frozen_string_literal: true\n")
      commands = []
      ...
      assert commands.last.include?("2024/02.rb"), "standardrb should target the solution file"
    end
  end

  def test_check_is_a_blocking_gate_on_standard_offenses
    Dir.mktmpdir do |dir|
      paths = AOC::Paths.new(root: dir, config_dir: File.join(dir, "config"))
      write_file(paths.day_path(2024, 2), "# frozen_string_literal: true\n")
      command_runner = ->(*command) { !command.include?("standardrb") }
  ...
  def test_ci_runs_runner_gate_then_solution_lint
    Dir.mktmpdir do |dir|
      ...
      standard_targets = commands.select { |command| command.first(3) == %w[bundle exec standardrb] }
        .map(&:last)
      assert standard_targets.any? { |target| target.start_with?("runner/") }, "ci lints the runner with Standard"
      assert_includes standard_targets, "2024/02.rb", "ci lints the solutions with Standard"
